...
export class Form implements Translatable {
  ...
  public constructor(species: PokemonSpecies, jsonData: JsonForm) {
    ...
    this.#battleStats = battleStatsValidator.nullish.parse(
      jsonData.battleStats,
    );
    this.#tags = s.array(s.string).unique.nullish.parse(jsonData.tags);
    this.#possibleGenders = s
      .array(s.string)
      .unique.nullish.parse(jsonData.possibleGenders);
    this.#genderProperties = genderPropertiesValidator.parse(
      jsonData.genderProperties,
    );
    this.#eggGroups = s
      .array(s.string)
      .unique.nullish.parse(jsonData.eggGroups);
    this.#types = s.array(s.string).unique.nullish.parse(jsonData.types);
    this.#preEvolutions = s
      .array(s.string)
      .unique.nullish.parse(jsonData.preEvolutions);
    this.#defaultBaseForm = s.string.nullish.parse(jsonData.defaultBaseForm);
    this.#eggCycles = s.number.positive.safeInt.nullish.parse(
      jsonData.eggCycles,
    );
    this.#catchRate = s.number.positive.safeInt.nullish.parse(
      jsonData.catchRate,
    );
    ...
    this.#evYields = battleStatsValidator.partial.nullish.parse(
      jsonData.evYields,
    );
  }

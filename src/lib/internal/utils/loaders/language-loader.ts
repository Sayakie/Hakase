...
export async function loadAllLanguages(): Promise<
  ReadonlyMap<`${Locale}`, Record<string, string>>
>;
export async function loadAllLanguages(
  targetDir: string = join(
    fileURLToPath(Directories.Data),
    ResourceIdentifiers.PIXELMON,
    "lang",
  ),
  ...
  for await (const languageFileName of languageFileNames) {
    ...
    if (ext !== ".json") {
      console.warn(
        `[LanguageLoader] Skip ${green(
          languageFileName,
        )} because could not transform to JSON.`,
      );
      continue;
    }
    ...
    const result = await Result.fromAsync(
      async () => await readFile(languageFilePath),
    );
    ...
    const jsonParseResult = Result.from(() =>
      JSON.parse(result.unwrap().toString()),
    );

    if (jsonParseResult.isErr()) {
      console.error(
        `[StatLoader] Failed to transform JSON data: ${languageFileName}`,
      );

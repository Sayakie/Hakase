...
export class SlashCommand extends LocalizableCommand {
  ...
  private get uptimeStatistics(): UptimeStatistics {
    ...
    return {
      client: time(new Date(now - this.container.client.uptime!), TimestampStyles.RelativeTime),
  ...
  private static formatCpuInfo({ times }: CpuInfo): `${string}%` {
    return `${~~(((times.user + times.nice + times.sys + times.irq) / times.idle) * 10000) / 100}%`
  }
